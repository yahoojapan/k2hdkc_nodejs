...
PrintUsage()
{
	...
	echo ""
	echo "Command: k2hdkc               k2hdkc test"
	echo ""
	echo "Option:"
	...
	echo "  --libdebuglevel(-ld) <mode> Specifies the debug level(DUMP / INFO / WARN / ERR / SILENT) for K2HDKC library.(default: SILENT)"
	echo "  --liblogfile(-ll) <path>    Specifies the file path for writing K2HDKC library debug messages."
	...
	echo ""
	echo "Note:"
	echo "  The file path is required to output K2HDKC library debug messages."
	echo "  If that file path is not specified, K2HDKC library debug level will not be set."
	echo "  If \"--libdebuglevel(-ld)\" is specified but \"--liblogfile(-ll)\" is not,"
	echo "  the value of \"--logfile(-l)\" will be used instead."
	echo "  You can also specify \"DKCDBGMODE\" and \"DKCDBGFILE\" environment variables"
...
#
if [ -z "${LIB_DEBUG_MODE}" ] && [ -n "${DKCDBGMODE}" ]; then
	# other than "SILENT"
	if echo "${DKCDBGMODE}" | grep -q -i -e "^dmp$" -e "^dump$"; then
		LIB_DEBUG_MODE="DUMP"
	elif echo "${DKCDBGMODE}" | grep -q -i -e "^inf$" -e "^info$" -e "^msg$"; then
		LIB_DEBUG_MODE="INFO"
	elif echo "${DKCDBGMODE}" | grep -q -i -e "^wan$" -e "^warn$" -e "^warning$"; then
		LIB_DEBUG_MODE="WAN"
	elif echo "${DKCDBGMODE}" | grep -q -i -e "^err$" -e "^error$"; then
		LIB_DEBUG_MODE="ERR"
	fi
fi
if [ -z "${LIB_DEBUG_LOG}" ] && [ -n "${DKCDBGFILE}" ]; then
	LIB_DEBUG_LOG="${DKCDBGFILE}"
fi

#
# Check debug mode/file for K2HDKC library
#
if [ -z "${LIB_DEBUG_MODE}" ]; then
	if [ -n "${LIB_DEBUG_LOG}" ]; then
		PRNERR "\"--libdebuglevel(-ld)\" or \"DKCDBGMODE\" is not set or \"SILENT\" is specified, but \"--liblogfile(-ll)\" or \"DKCDBGFILE\" is set."
		exit 1
	fi
	...
	if [ -z "${LIB_DEBUG_LOG}" ]; then
		...
		if [ -z "${SCRIPT_DEBUG_LOG}" ]; then
			PRNERR "\"--libdebuglevel(-ld)\" or \"DKCDBGMODE\" is specified(other than \"SILENT\"), but \"--liblogfile(-ll)\" or \"DKCDBGFILE\" or \"SCRIPT_DEBUG_LOG\" is not set."
			exit 1
		fi
...
#----------------------------------------------------------
# [NOTE]
# This test needs to import(require) k2hdkc to run, which
# requires it to exist as a package under node_modules(read
# from package.json).
# Since it won't exist during development, temporarily create
# node_modules/k2hdkc before running the tests.
#
# By making the following settings, the following execution
# will succeed:
#	node -e "import('k2hdkc').then(m=>console.log('import ok', Object.keys(m))).catch(e=>console.error(e))"
#

#
# Create node_modules/k2hdkc directory
#
if [ -f "${SRCTOP}/node_modules/k2hdkc" ] || [ -d "${SRCTOP}/node_modules/k2hdkc" ]; then
	PRNINFO "Found node_modules/k2hdkc, so remove it and re-create it."
	rm -rf "${SRCTOP}/node_modules/k2hdkc" >/dev/null 2>&1
fi
if ! mkdir -p "${SRCTOP}/node_modules/k2hdkc" >/dev/null 2>&1; then
	PRNERR "Could not create node_modules/k2hdkc directory."
	exit 1
fi
...
#
{
	...
	echo '  }'
	echo '}'
} > "${SRCTOP}/node_modules/k2hdkc/package.json" 2>/dev/null || (PRNERR "Could not create node_modules/k2hdkc/package.json dummy file."; rm -rf "${SRCTOP}/node_modules/k2hdkc"; exit 1)
...
} > "${SRCTOP}/node_modules/k2hdkc/index.cjs" 2>/dev/null || (PRNERR "Could not create node_modules/k2hdkc/index.cjs dummy file."; rm -rf "${SRCTOP}/node_modules/k2hdkc"; exit 1)
...
} > "${SRCTOP}/node_modules/k2hdkc/index.mjs" 2>/dev/null || (PRNERR "Could not create node_modules/k2hdkc/index.mjs dummy file."; rm -rf "${SRCTOP}/node_modules/k2hdkc"; exit 1)
...
if ! npm list mocha --depth=0 >/dev/null 2>&1; then
	...
	if ! npm install mocha >/dev/null 2>&1; then
		...
		rm -rf "${SRCTOP}/node_modules/k2hdkc"
		exit 1
	fi
fi
...
PRNINFO "Run : NODE_PATH=${K2HDKC_NODE_PATH} TESTS_PATH=${TESTSDIR} SCRIPT_TYPE=${SCRIPT_TYPE} DKCDBGMODE=${LIB_DEBUG_MODE} DKCDBGFILE=${LIB_DEBUG_LOG} node ${ESM_IMPORT_OPT} ${ESM_EXPR_OPT} ${MOCHA_BIN} --extensions ts ${TEST_FILE_DIR_PATH}/${CMD_PREFIX}${COMMAND}${TEST_FILE_SUFFIX}"

if [ -n "${SCRIPT_DEBUG_LOG}" ]; then
	if ! /bin/sh -c "NODE_PATH=${K2HDKC_NODE_PATH} TESTS_PATH=${TESTSDIR} SCRIPT_TYPE=${SCRIPT_TYPE} DKCDBGMODE=${LIB_DEBUG_MODE} DKCDBGFILE=${LIB_DEBUG_LOG} node ${ESM_IMPORT_OPT} ${ESM_EXPR_OPT} ${MOCHA_BIN} --extensions ts ${TEST_FILE_DIR_PATH}/${CMD_PREFIX}${COMMAND}${TEST_FILE_SUFFIX} >>${SCRIPT_DEBUG_LOG} 2>&1"; then
		PRNFAILURE "${COMMAND} test"
		rm -rf "${SRCTOP}/node_modules/k2hdkc"
		exit 1
	fi
else
	if ! /bin/sh -c "NODE_PATH=${K2HDKC_NODE_PATH} TESTS_PATH=${TESTSDIR} SCRIPT_TYPE=${SCRIPT_TYPE} DKCDBGMODE=${LIB_DEBUG_MODE} DKCDBGFILE=${LIB_DEBUG_LOG} node ${ESM_IMPORT_OPT} ${ESM_EXPR_OPT} ${MOCHA_BIN} --extensions ts ${TEST_FILE_DIR_PATH}/${CMD_PREFIX}${COMMAND}${TEST_FILE_SUFFIX}"; then
		PRNFAILURE "${COMMAND} test"
		rm -rf "${SRCTOP}/node_modules/k2hdkc"
		exit 1
	fi
