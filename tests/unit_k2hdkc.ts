...
import	* as _k2hdkc			from 'k2hdkc';
const	k2hdkc: any				= (_k2hdkc as any).default ?? _k2hdkc;
...
//
const startAllSubProcesses = (parentobj: ParentWithTimeout, testsdir: string): void =>
{
	...
	//
	const	result = execSync(testsdir + '/run_process_helper.sh start_all');
...
//
const stopAllSubProcesses = (parentobj: ParentWithTimeout, testsdir: string): void =>
{
	console.log('');
	...
	//
	const	result = execSync(testsdir + '/run_process_helper.sh stop_all');
	console.log(String(result).replace(/\r?\n$/g, ''));
	console.log('');
...
//--------------------------------------------------------------
describe('K2HDKC', function(){
	//
	// Global
	//
	const	slvconf	= testsdir + '/test_slave.ini';
	const	slvport	= 8031;
	...
	before(function(done){
		startAllSubProcesses(this, testsdir);
		done();
	});
	...
	//
	after(function(done){
		stopAllSubProcesses(this, testsdir);
		done();
	});
	...
	//
	it('k2hdkc - k2hash::k2hdkc(param) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hash::k2hdkc(new param) - No Callback', function(done){
		const	dkcobj = new k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hash::k2hdkc(copy param) - No Callback', function(done){
		const	dkcobj1 = new k2hdkc(slvconf, slvport, true, false);
		const	dkcobj2 = dkcobj1;
	...
	//
	it('k2hdkc - k2hash::init(no param) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hash::init(no param) - on Callback', function(done){
		const	dkcobj = k2hdkc();

		// Registered on callback
		expect(dkcobj.on('init', function(error: any)
		{
			expect(error).to.equal('Error: No configuration is associated to async worker');
			dkcobj.off('init');
	...
	//
	it('k2hdkc - k2hash::init(no param) - onXXXX Callback', function(done){
		const	dkcobj = k2hdkc();

		// Registered on callback
		expect(dkcobj.onInit(function(error: any)
		{
			expect(error).to.equal('Error: No configuration is associated to async worker');
			dkcobj.offInit();
	...
	//
	it('k2hdkc - k2hash::init(param) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hash::init(param) - on Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hash::init(param) - onXXXX Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hash::init(param) - inline Callback', function(done){
		const	dkcobj = k2hdkc();

		// inline callback
		expect(dkcobj.init(slvconf, slvport, true, false, function(error: any)
		{
			expect(error).to.be.null;
			expect(dkcobj.isPermanent()).to.be.a('boolean').to.be.true;
	...
	//
	it('k2hdkc - k2hdkc::GetValue/SetValue(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::GetValue/SetValue(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();

		// Registered on(set) callback
		expect(dkcobj.on('set', function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.off('set');

			// Registered on(get - key) callback
			expect(dkcobj.on('get', function(error: any, value?: string | null)
			{
				expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::GetValue/SetValue(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();

		// Registered onSet callback
		expect(dkcobj.onSet(function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.offSet();

			// Registered onGet(key) callback
			expect(dkcobj.onGet(function(error: any, value?: string | null)
			{
				expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::GetValue/SetValue(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();

		// inline setValue callback
		expect(dkcobj.setValue(slvconf, slvport, true, false, 'getvalue-onetime-key-004', 'getvalue-onetime-subval-004', 'getvalue-onetime-subkey-004', null, 0, function(error: any)
		{
			expect(error).to.be.null;

			// inline getValue(key) callback
			expect(dkcobj.getValue(slvconf, slvport, true, false, 'getvalue-onetime-subkey-004', null, true, null, function(error: any, value?: string | null)
			{
				expect(error).to.be.null;
				expect(value).to.equal('getvalue-onetime-subval-004');

				// inline getValue(subkey) callback
				expect(dkcobj.getValue(slvconf, slvport, true, false, 'getvalue-onetime-key-004', 'getvalue-onetime-subkey-004', true, null, function(error: any, value?: string | null)
				{
					expect(error).to.be.null;
					expect(value).to.equal('getvalue-onetime-subval-004');
	...
	//
	it('k2hdkc - k2hdkc::GetValue/SetValue(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::GetValue/SetValue(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::GetValue/SetValue(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::GetValue/SetValue(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);

		// inline setValue callback
		expect(dkcobj.setValue('getvalue-permanent-key-004', 'getvalue-permanent-subval-004', 'getvalue-permanent-subkey-004', null, 0, function(error: any)
		{
			expect(error).to.be.null;

			// inline getValue(key) callback
			expect(dkcobj.getValue('getvalue-permanent-subkey-004', null, true, null, function(error: any, value?: string | null)
			{
				expect(error).to.be.null;
				expect(value).to.equal('getvalue-permanent-subval-004');

				// inline getValue(subkey) callback
				expect(dkcobj.getValue('getvalue-permanent-key-004', 'getvalue-permanent-subkey-004', true, null, function(error: any, value?: string | null)
				{
					expect(error).to.be.null;
					expect(value).to.equal('getvalue-permanent-subval-004');
	...
	//
	it('k2hdkc - k2hdkc::GetSubkeys/SetSubkeys(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// set subkeys[2]
		const	subkeys = ['getsubkeys-onetime-subkey-101', 'getsubkeys-onetime-subkey-102'];
		expect(dkcobj.setSubkeys(slvconf, slvport, true, false, 'getsubkeys-onetime-key-101', subkeys)).to.be.a('boolean').to.be.true;

		// get subkeys
		const	skeylist = dkcobj.getSubkeys(slvconf, slvport, true, false, 'getsubkeys-onetime-key-101', true);
	...
	//
	it('k2hdkc - k2hdkc::GetSubkeys/SetSubkeys(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.on('setSubkeys', function(error: any)
		{
			expect(error).to.be.null;
		...
		expect(dkcobj.on('getSubkeys', function(error: any, skeylist?: string[] | null)
		{
			expect(error).to.be.null;
			...
			expect((skeylist as string[])[0]).to.equal('getsubkeys-onetime-subkey-111');
			expect((skeylist as string[])[1]).to.equal('getsubkeys-onetime-subkey-112');
		...
		// set subkeys[2]
		const	subkeys = ['getsubkeys-onetime-subkey-111', 'getsubkeys-onetime-subkey-112'];
	...
	//
	it('k2hdkc - k2hdkc::GetSubkeys/SetSubkeys(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.onSetSubkeys(function(error: any)
		{
			expect(error).to.be.null;
		...
		expect(dkcobj.onGetSubkeys(function(error: any, skeylist?: string[] | null)
		{
			expect(error).to.be.null;
			...
			expect((skeylist as string[]).length).to.equal(2);
			expect((skeylist as string[])[0]).to.equal('getsubkeys-onetime-subkey-121');
			expect((skeylist as string[])[1]).to.equal('getsubkeys-onetime-subkey-122');
		...
		// set subkeys[2]
		const	subkeys = ['getsubkeys-onetime-subkey-121', 'getsubkeys-onetime-subkey-122'];
	...
	//
	it('k2hdkc - k2hdkc::GetSubkeys/SetSubkeys(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// set subkeys[2]
		const	subkeys = ['getsubkeys-onetime-subkey-131', 'getsubkeys-onetime-subkey-132'];
		expect(dkcobj.setSubkeys(slvconf, slvport, true, false, 'getsubkeys-onetime-key-131', subkeys, function(error: any)
		{
			expect(error).to.be.null;

			// get subkeys
			expect(dkcobj.getSubkeys(slvconf, slvport, true, false, 'getsubkeys-onetime-key-131', true, function(error: any, skeylist?: string[] | null)
			{
				expect(error).to.be.null;
				...
				expect((skeylist as string[]).length).to.equal(2);
				expect((skeylist as string[])[0]).to.equal('getsubkeys-onetime-subkey-131');
				expect((skeylist as string[])[1]).to.equal('getsubkeys-onetime-subkey-132');
	...
	//
	it('k2hdkc - k2hdkc::GetSubkeys/SetSubkeys(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// set subkeys[2]
		const	subkeys = ['getsubkeys-permanent-subkey-151', 'getsubkeys-permanent-subkey-152'];
		expect(dkcobj.setSubkeys('getsubkeys-permanent-key-151', subkeys)).to.be.a('boolean').to.be.true;

		// get subkeys
		const	skeylist: string[] | null = dkcobj.getSubkeys('getsubkeys-permanent-key-151', true);
		expect(skeylist instanceof Array, 'k2hdkc::GetSubkeys/SetSubkeys(permanent) - No Callback : getSubkeys() : got not array');
		expect((skeylist as string[]).length).to.equal(2);
		expect((skeylist as string[])[0]).to.equal('getsubkeys-permanent-subkey-151');
		expect((skeylist as string[])[1]).to.equal('getsubkeys-permanent-subkey-152');
	...
	//
	it('k2hdkc - k2hdkc::GetSubkeys/SetSubkeys(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		expect(dkcobj.on('getSubkeys', function(error: any, skeylist?: string[] | null)
		{
			...
			expect((skeylist as string[]).length).to.equal(2);
			expect((skeylist as string[])[0]).to.equal('getsubkeys-permanent-subkey-161');
			expect((skeylist as string[])[1]).to.equal('getsubkeys-permanent-subkey-162');
		...
		// set subkeys[2]
		const	subkeys = ['getsubkeys-permanent-subkey-161', 'getsubkeys-permanent-subkey-162'];
	...
	//
	it('k2hdkc - k2hdkc::GetSubkeys/SetSubkeys(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		expect(dkcobj.onGetSubkeys(function(error: any, skeylist?: string[] | null)
		{
			...
			expect((skeylist as string[]).length).to.equal(2);
			expect((skeylist as string[])[0]).to.equal('getsubkeys-permanent-subkey-171');
			expect((skeylist as string[])[1]).to.equal('getsubkeys-permanent-subkey-172');
		...
		// set subkeys[2]
		const	subkeys = ['getsubkeys-permanent-subkey-171', 'getsubkeys-permanent-subkey-172'];
	...
	//
	it('k2hdkc - k2hdkc::GetSubkeys/SetSubkeys(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// set subkeys[2]
		const	subkeys = ['getsubkeys-permanent-subkey-181', 'getsubkeys-permanent-subkey-182'];
		expect(dkcobj.setSubkeys('getsubkeys-permanent-key-181', subkeys, function(error: any)
		{
			expect(error).to.be.null;

			// get subkeys
			expect(dkcobj.getSubkeys('getsubkeys-permanent-key-181', true, function(error: any, skeylist?: string[] | null)
			{
				expect(error).to.be.null;
				...
				expect((skeylist as string[]).length).to.equal(2);
				expect((skeylist as string[])[0]).to.equal('getsubkeys-permanent-subkey-181');
				expect((skeylist as string[])[1]).to.equal('getsubkeys-permanent-subkey-182');
	...
	//
	it('k2hdkc - k2hdkc::GetAttrs(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// get attributes
		const	attrlist: string[] | null = dkcobj.getAttrs(slvconf, slvport, true, false, 'getattrs-onetime-key-201');
		expect(attrlist instanceof Array, 'k2hdkc::GetAttrs(onetime) - No Callback : getAttrs() : got not array');
		expect((attrlist as string[]).length).to.equal(3);
		expect((attrlist as string[])[0]).to.equal('aes256_cbc_pad.pbkdf2.sha256');
		expect((attrlist as string[])[1]).to.equal('expire');
		expect((attrlist as string[])[2]).to.equal('mtime');
	...
	//
	it('k2hdkc - k2hdkc::GetAttrs(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.on('getAttrs', function(error: any, attrlist?: string[] | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::GetAttrs(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.onGetAttrs(function(error: any, attrlist?: string[] | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::GetAttrs(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// get attributes
		expect(dkcobj.getAttrs(slvconf, slvport, true, false, 'getattrs-onetime-key-231', function(error: any, attrlist?: string[] | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::GetAttrs(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// get attributes
		const	attrlist: string[] | null = dkcobj.getAttrs('getattrs-permanent-key-251');
	...
	//
	it('k2hdkc - k2hdkc::GetAttrs(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::GetAttrs(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::GetAttrs(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// get attributes
		expect(dkcobj.getAttrs('getattrs-permanent-key-281', function(error: any, attrlist?: string[] | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::SetAll(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// set all
		const	subkeys = ['setall-onetime-subkey-301'];
	...
	//
	it('k2hdkc - k2hdkc::SetAll(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();

		// Registered on(setAll) callback
		expect(dkcobj.on('setAll', function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.off('setAll');
		...
		// set all
		const	subkeys = ['setall-onetime-subkey-302'];
	...
	//
	it('k2hdkc - k2hdkc::SetAll(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();

		// Registered onSetAll callback
		expect(dkcobj.onSetAll(function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.offSet();
		...
		// set all
		const	subkeys = ['setall-onetime-subkey-303'];
	...
	//
	it('k2hdkc - k2hdkc::SetAll(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		const	subkeys = ['setall-onetime-subkey-304'];
		expect(dkcobj.setAll(slvconf, slvport, true, false, 'setall-onetime-key-304', 'setall-onetime-val-304', subkeys, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::SetAll(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// set all
		const	subkeys = ['setall-permanent-subkey-305'];
	...
	//
	it('k2hdkc - k2hdkc::SetAll(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// set all
		const	subkeys = ['setall-permanent-subkey-306'];
	...
	//
	it('k2hdkc - k2hdkc::SetAll(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// set all
		const	subkeys = ['setall-permanent-subkey-307'];
	...
	//
	it('k2hdkc - k2hdkc::SetAll(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// inline setAll callback
		const	subkeys = ['setall-permanent-subkey-308'];
		expect(dkcobj.setAll('setall-permanent-key-308', 'setall-permanent-val-308', subkeys, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::Remove(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::Remove(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.on('remove', function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.off('remove');
	...
	//
	it('k2hdkc - k2hdkc::Remove(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.onRemove(function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.offRemove();
	...
	//
	it('k2hdkc - k2hdkc::Remove(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// remove key
		expect(dkcobj.remove(slvconf, slvport, true, false, 'remove-onetime-key-431', false, function(error: any)
		{
			expect(error).to.be.null;

			// remove key
			expect(dkcobj.remove(slvconf, slvport, true, false, 'remove-onetime-key-432', true, function(error: any)
			{
				expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::Remove(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::Remove(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::Remove(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::Remove(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// remove key
		expect(dkcobj.remove('remove-permanent-key-481', false, function(error: any)
		{
			expect(error).to.be.null;

			// remove key
			expect(dkcobj.remove('remove-permanent-key-482', true, function(error: any)
			{
				expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::Rename(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// check
		const	skeylist: string[] | null = dkcobj.getSubkeys(slvconf, slvport, true, false, 'rename-onetime-key-501', true);
		...
		expect((skeylist as string[])[0]).to.equal('rename-onetime-subkey-502');
	...
	//
	it('k2hdkc - k2hdkc::Rename(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.on('rename', function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.off('rename');

			// check
			const	skeylist: string[] | null = dkcobj.getSubkeys(slvconf, slvport, true, false, 'rename-onetime-key-511', true);
			expect(skeylist instanceof Array, 'k2hdkc::Rename(onetime) - on Callback : getSubkeys got not array');
			expect((skeylist as string[]).length).to.equal(1);
			expect((skeylist as string[])[0]).to.equal('rename-onetime-subkey-512');
	...
	//
	it('k2hdkc - k2hdkc::Rename(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.onRename(function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.offRename();

			// check
			const	skeylist: string[] | null = dkcobj.getSubkeys(slvconf, slvport, true, false, 'rename-onetime-key-521', true);
			expect(skeylist instanceof Array, 'k2hdkc::Rename(onetime) - onXXX Callback : getSubkeys got not array');
			expect((skeylist as string[]).length).to.equal(1);
			expect((skeylist as string[])[0]).to.equal('rename-onetime-subkey-522');
	...
	//
	it('k2hdkc - k2hdkc::Rename(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// inline callback
		expect(dkcobj.rename(slvconf, slvport, true, false, 'rename-onetime-subkey-531', 'rename-onetime-subkey-532', 'rename-onetime-key-531', true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;

			// check
			const	skeylist: string[] | null = dkcobj.getSubkeys(slvconf, slvport, true, false, 'rename-onetime-key-531', true);
			expect(skeylist instanceof Array, 'k2hdkc::Rename(onetime) - inline Callback : getSubkeys got not array');
			expect((skeylist as string[]).length).to.equal(1);
			expect((skeylist as string[])[0]).to.equal('rename-onetime-subkey-532');
	...
	//
	it('k2hdkc - k2hdkc::Rename(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// check
		const	skeylist: string[] | null = dkcobj.getSubkeys('rename-permanent-key-551', true);
		expect(skeylist instanceof Array, 'k2hdkc::Rename(permanent) - No Callback : getSubkeys got not array');
		expect((skeylist as string[]).length).to.equal(1);
		expect((skeylist as string[])[0]).to.equal('rename-permanent-subkey-552');
	...
	//
	it('k2hdkc - k2hdkc::Rename(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		expect(dkcobj.on('rename', function(error: any)
		{
			...
			// check
			const	skeylist: string[] | null = dkcobj.getSubkeys('rename-permanent-key-561', true);
			expect(skeylist instanceof Array, 'k2hdkc::Rename(permanent) - on Callback : getSubkeys got not array');
			expect((skeylist as string[]).length).to.equal(1);
			expect((skeylist as string[])[0]).to.equal('rename-permanent-subkey-562');
	...
	//
	it('k2hdkc - k2hdkc::Rename(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		expect(dkcobj.onRename(function(error: any)
		{
			...
			// check
			const	skeylist: string[] | null = dkcobj.getSubkeys('rename-permanent-key-571', true);
			expect(skeylist instanceof Array, 'k2hdkc::Rename(permanent) - onXXX Callback : getSubkeys got not array');
			expect((skeylist as string[]).length).to.equal(1);
			expect((skeylist as string[])[0]).to.equal('rename-permanent-subkey-572');
	...
	//
	it('k2hdkc - k2hdkc::Rename(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// inline callback
		expect(dkcobj.rename('rename-permanent-subkey-581', 'rename-permanent-subkey-582', 'rename-permanent-key-581', true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;

			// check
			const	skeylist: string[] | null = dkcobj.getSubkeys('rename-permanent-key-581', true);
			expect(skeylist instanceof Array, 'k2hdkc::Rename(permanent) - inline Callback : getSubkeys got not array');
			expect((skeylist as string[]).length).to.equal(1);
			expect((skeylist as string[])[0]).to.equal('rename-permanent-subkey-582');
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePush(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePush(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();

		// Registered on(queuePush) callback
		expect(dkcobj.on('queuePush', function(error: any)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePush(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();

		// Registered on(queuePush) callback
		expect(dkcobj.onQueuePush(function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.offQueuePush();
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePush(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();

		// push
		expect(dkcobj.queuePush(slvconf, slvport, true, false, 'queuepush-onetime-prefix-604-', null, 'queuepush-onetime-value-604-fifo', true, true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePush(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePush(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePush(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePush(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);

		// push
		expect(dkcobj.queuePush('queuepush-permanent-prefix-608-', null, 'queuepush-permanent-value-608-fifo', true, true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePush(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePush(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePush(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePush(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();

		// push
		expect(dkcobj.queuePush(slvconf, slvport, true, false, 'queuepush-onetime-prefix-614-', null, 'queuepush-onetime-value-614-lifo', false, true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePush(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePush(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePush(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePush(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);

		// push
		expect(dkcobj.queuePush('queuepush-permanent-prefix-618-', null, 'queuepush-permanent-value-618-lifo', false, true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePop(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePop(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.on('queuePop', function(error: any, value?: string | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePop(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.onQueuePop(function(error: any, value?: string | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePop(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// pop callback
		expect(dkcobj.queuePop(slvconf, slvport, true, false, 'queuepop-onetime-prefix-624-', true, false, 'mypass', function(error: any, value?: string | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePop(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePop(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePop(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueuePop(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// pop callback
		expect(dkcobj.queuePop('queuepop-permanent-prefix-628-', true, false, 'mypass', function(error: any, value?: string | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePop(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePop(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePop(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePop(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// pop callback
		expect(dkcobj.queuePop(slvconf, slvport, true, false, 'queuepop-onetime-prefix-634-', false, false, 'mypass', function(error: any, value?: string | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePop(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePop(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePop(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueuePop(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// pop callback
		expect(dkcobj.queuePop('queuepop-permanent-prefix-638-', false, false, 'mypass', function(error: any, value?: string | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueueRemove(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueueRemove(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.on('queueRemove', function(error: any)
		{
			expect(error).to.be.null;
			...
			expect(dkcobj.on('queueRemove', function(error: any)
			{
				...
				expect(dkcobj.on('queueRemove', function(error: any)
				{
					expect(error).to.equal('Error: Failed to remove queue.');
					dkcobj.off('queueRemove');
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueueRemove(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.onQueueRemove(function(error: any)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueueRemove(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// remove
		expect(dkcobj.queueRemove(slvconf, slvport, true, false, 'queueremove-onetime-prefix-644-', 1, true, false, 'mypass', function(error: any)
		{
			expect(error).to.be.null;

			// remove
			expect(dkcobj.queueRemove(slvconf, slvport, true, false, 'queueremove-onetime-prefix-644-', 2, true, false, 'mypass', function(error: any)
			{
				expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueueRemove(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueueRemove(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueueRemove(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-QueueRemove(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// remove
		expect(dkcobj.queueRemove('queueremove-permanent-prefix-648-', 1, true, false, 'mypass', function(error: any)
		{
			expect(error).to.be.null;

			// remove
			expect(dkcobj.queueRemove('queueremove-permanent-prefix-648-', 2, true, false, 'mypass', function(error: any)
			{
				expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueueRemove(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueueRemove(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueueRemove(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueueRemove(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// remove
		expect(dkcobj.queueRemove(slvconf, slvport, true, false, 'queueremove-onetime-prefix-744-', 1, false, false, 'mypass', function(error: any)
		{
			expect(error).to.be.null;

			// remove
			expect(dkcobj.queueRemove(slvconf, slvport, true, false, 'queueremove-onetime-prefix-744-', 2, false, false, 'mypass', function(error: any)
			{
				expect(error).to.be.null;
				dkcobj.offQueueRemove();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueueRemove(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueueRemove(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueueRemove(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-QueueRemove(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// remove
		expect(dkcobj.queueRemove('queueremove-permanent-prefix-748-', 1, false, false, 'mypass', function(error: any)
		{
			expect(error).to.be.null;

			// remove
			expect(dkcobj.queueRemove('queueremove-permanent-prefix-748-', 2, false, false, 'mypass', function(error: any)
			{
				expect(error).to.be.null;
				dkcobj.offQueueRemove();
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePush(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// check(pop)
		const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'queuepush-onetime-prefix-801-', true, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePush(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.on('queuePush', function(error: any)
		{
			...
			// check(pop)
			const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'queuepush-onetime-prefix-802-', true, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePush(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.onQueuePush(function(error: any)
		{
			...
			// check(pop)
			const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'queuepush-onetime-prefix-803-', true, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePush(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();

		// push
		expect(dkcobj.queuePush(slvconf, slvport, true, false, 'queuepush-onetime-prefix-804-', 'queuepush-onetime-key-804-fifo-a', 'queuepush-onetime-value-804-fifo', true, true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;

			// check(pop)
			const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'queuepush-onetime-prefix-804-', true, true, 'mypass');
	...
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePush(onetime) - No Callback', function(done){
		...
		// check(pop)
		const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'kqueuepush-onetime-prefix-801-', true, true, 'mypass');
		expect(queuelist instanceof Array, 'k2hdkc::FIFO-KeyQueuePush(onetime) - No Callback : QueuePop got not array');
	...
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePush(onetime) - on Callback', function(done){
		...
		expect(dkcobj.on('queuePush', function(error: any)
		{
			...
			// check(pop)
			const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'kqueuepush-onetime-prefix-802-', true, true, 'mypass');
			expect(queuelist instanceof Array, 'k2hdkc::FIFO-KeyQueuePush(onetime) - on Callback : QueuePop got not array');
	...
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePush(onetime) - onXXX Callback', function(done){
		...
		expect(dkcobj.onQueuePush(function(error: any)
		{
			...
			// check(pop)
			const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'kqueuepush-onetime-prefix-803-', true, true, 'mypass');
			expect(queuelist instanceof Array, 'k2hdkc::FIFO-KeyQueuePush(onetime) - onXXX Callback : QueuePop got not array');
	...
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePush(onetime) - inline Callback', function(done){
		...
		// push
		expect(dkcobj.queuePush(slvconf, slvport, true, false, 'kqueuepush-onetime-prefix-804-', 'kqueuepush-onetime-key-804-fifo-a', 'kqueuepush-onetime-value-804-fifo', true, true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;

			// check(pop)
			const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'kqueuepush-onetime-prefix-804-', true, true, 'mypass');
			expect(queuelist instanceof Array, 'k2hdkc::FIFO-KeyQueuePush(onetime) - inline Callback : QueuePop got not array');
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePush(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// check(pop)
		const	queuelist = dkcobj.queuePop('kqueuepush-permanent-prefix-805-', true, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePush(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		expect(dkcobj.on('queuePush', function(error: any)
		{
			...
			// check(pop)
			const	queuelist = dkcobj.queuePop('kqueuepush-permanent-prefix-806-', true, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePush(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		expect(dkcobj.onQueuePush(function(error: any)
		{
			...
			// check(pop)
			const	queuelist = dkcobj.queuePop('kqueuepush-permanent-prefix-807-', true, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePush(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);

		// push
		expect(dkcobj.queuePush('kqueuepush-permanent-prefix-808-', 'kqueuepush-permanent-key-808-fifo-a', 'kqueuepush-permanent-value-808-fifo', true, true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;

			// check(pop)
			const	queuelist = dkcobj.queuePop('kqueuepush-permanent-prefix-808-', true, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePush(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// check(pop)
		const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'kqueuepush-onetime-prefix-811-', false, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePush(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.on('queuePush', function(error: any)
		{
			...
			// check(pop)
			const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'kqueuepush-onetime-prefix-812-', false, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePush(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.onQueuePush(function(error: any)
		{
			...
			// check(pop)
			const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'kqueuepush-onetime-prefix-813-', false, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePush(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();

		// push
		expect(dkcobj.queuePush(slvconf, slvport, true, false, 'kqueuepush-onetime-prefix-814-', 'kqueuepush-onetime-key-814-lifo-a', 'kqueuepush-onetime-value-814-lifo', false, true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;

			// check(pop)
			const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'kqueuepush-onetime-prefix-814-', false, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePush(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// check(pop)
		const	queuelist = dkcobj.queuePop('kqueuepush-permanent-prefix-815-', false, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePush(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		expect(dkcobj.on('queuePush', function(error: any)
		{
			...
			// check(pop)
			const	queuelist = dkcobj.queuePop('kqueuepush-permanent-prefix-816-', false, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePush(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		expect(dkcobj.onQueuePush(function(error: any)
		{
			...
			// check(pop)
			const	queuelist = dkcobj.queuePop('kqueuepush-permanent-prefix-817-', false, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePush(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);

		// push
		expect(dkcobj.queuePush('kqueuepush-permanent-prefix-818-', 'kqueuepush-permanent-key-818-lifo-a', 'kqueuepush-permanent-value-818-lifo', false, true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;

			// check(pop)
			const	queuelist = dkcobj.queuePop('kqueuepush-permanent-prefix-818-', false, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePop(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// check(pop)
		const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'kqueuepop-onetime-prefix-821-', true, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePop(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// Registered on(queuePop) callback
		expect(dkcobj.on('queuePop', function(error: any, key?: string | null, value?: string | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePop(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// Registered onQueuePop callback
		expect(dkcobj.onQueuePop(function(error: any, key?: string | null, value?: string | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePop(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// pop
		expect(dkcobj.queuePop(slvconf, slvport, true, false, 'kqueuepop-onetime-prefix-824-', true, true, 'mypass', function(error: any, key?: string | null, value?: string | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePop(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// check(pop)
		const	queuelist = dkcobj.queuePop('kqueuepop-permanent-prefix-825-', true, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePop(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePop(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueuePop(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// pop
		expect(dkcobj.queuePop('kqueuepop-permanent-prefix-828-', true, true, 'mypass', function(error: any, key?: string | null, value?: string | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePop(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// check(pop)
		const	queuelist = dkcobj.queuePop(slvconf, slvport, true, false, 'kqueuepop-onetime-prefix-831-', false, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePop(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePop(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePop(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// pop
		expect(dkcobj.queuePop(slvconf, slvport, true, false, 'kqueuepop-onetime-prefix-834-', false, true, 'mypass', function(error: any, key?: string | null, value?: string | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePop(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// check(pop)
		const	queuelist = dkcobj.queuePop('kqueuepop-permanent-prefix-835-', false, true, 'mypass');
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePop(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePop(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueuePop(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// pop
		expect(dkcobj.queuePop('kqueuepop-permanent-prefix-838-', false, true, 'mypass', function(error: any, key?: string | null, value?: string | null)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueueRemove(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueueRemove(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueueRemove(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueueRemove(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// remove
		expect(dkcobj.queueRemove(slvconf, slvport, true, false, 'kqueueremove-onetime-prefix-844-', 1, true, true, 'mypass', function(error: any)
		{
			expect(error).to.be.null;

			// remove
			expect(dkcobj.queueRemove(slvconf, slvport, true, false, 'kqueueremove-onetime-prefix-844-', 2, true, true, 'mypass', function(error: any)
			{
				expect(error).to.be.null;
				dkcobj.offQueueRemove();
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueueRemove(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueueRemove(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueueRemove(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::FIFO-KeyQueueRemove(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// remove
		expect(dkcobj.queueRemove('kqueueremove-permanent-prefix-848-', 1, true, true, 'mypass', function(error: any)
		{
			expect(error).to.be.null;

			// remove
			expect(dkcobj.queueRemove('kqueueremove-permanent-prefix-848-', 2, true, true, 'mypass', function(error: any)
			{
				expect(error).to.be.null;
				dkcobj.offQueueRemove();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueueRemove(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueueRemove(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueueRemove(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueueRemove(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// remove
		expect(dkcobj.queueRemove(slvconf, slvport, true, false, 'kqueueremove-onetime-prefix-854-', 1, false, true, 'mypass', function(error: any)
		{
			expect(error).to.be.null;

			// remove
			expect(dkcobj.queueRemove(slvconf, slvport, true, false, 'kqueueremove-onetime-prefix-854-', 2, false, true, 'mypass', function(error: any)
			{
				expect(error).to.be.null;
				dkcobj.offQueueRemove();
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueueRemove(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueueRemove(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueueRemove(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::LIFO-KeyQueueRemove(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// remove
		expect(dkcobj.queueRemove('kqueueremove-permanent-prefix-858-', 1, false, true, 'mypass', function(error: any)
		{
			expect(error).to.be.null;

			// remove
			expect(dkcobj.queueRemove('kqueueremove-permanent-prefix-858-', 2, false, true, 'mypass', function(error: any)
			{
				expect(error).to.be.null;
				dkcobj.offQueueRemove();
	...
	//
	it('k2hdkc - k2hdkc::CasInit(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::CasInit(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();

		// Registered on(casInit) callback
		expect(dkcobj.on('casInit', function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.off('casInit');
	...
	//
	it('k2hdkc - k2hdkc::CasInit(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();

		// Registered onCasInit callback
		expect(dkcobj.onCasInit(function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.offCasInit();
	...
	//
	it('k2hdkc - k2hdkc::CasInit(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();

		// cas value init
		expect(dkcobj.casInit(slvconf, slvport, true, false, 'casinit-onetime-key-904', 1234, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::CasInit(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::CasInit(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::CasInit(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::CasInit(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);

		// cas value init
		expect(dkcobj.casInit('casinit-permanent-key-908', 1234, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::CasSet/CasGet(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::CasSet/CasGet(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.on('casSet', function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.off('casSet');
		...
		expect(dkcobj.on('casGet', function(error: any, value?: number)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::CasSet/CasGet(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.onCasSet(function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.offCasSet();
		...
		expect(dkcobj.onCasGet(function(error: any, value?: number)
		{
			expect(error).to.be.null;
			expect(value).to.equal(4321);
			dkcobj.offCasGet();
	...
	//
	it('k2hdkc - k2hdkc::CasSet/CasGet(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// set cas value
		expect(dkcobj.casSet(slvconf, slvport, true, false, 'casinit-onetime-key-914', 1234, 4321, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;

			// get cas value
			expect(dkcobj.casGet(slvconf, slvport, true, false, 'casinit-onetime-key-914', 'mypass', function(error: any, value?: number)
			{
				expect(error).to.be.null;
				expect(value).to.equal(4321);
	...
	//
	it('k2hdkc - k2hdkc::CasSet/CasGet(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::CasSet/CasGet(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::CasSet/CasGet(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::CasSet/CasGet(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// set cas value
		expect(dkcobj.casSet('casinit-permanent-key-918', 1234, 4321, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;

			// get cas value
			expect(dkcobj.casGet('casinit-permanent-key-918', 'mypass', function(error: any, value?: number)
			{
				expect(error).to.be.null;
				expect(value).to.equal(4321);
	...
	//
	it('k2hdkc - k2hdkc::CasIncDec(onetime) - No Callback', function(done){
		const	dkcobj = k2hdkc();
	...
	//
	it('k2hdkc - k2hdkc::CasIncDec(onetime) - on Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.on('casIncDec', function(error: any)
		{
			expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::CasIncDec(onetime) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		expect(dkcobj.onCasIncDec(function(error: any)
		{
			expect(error).to.be.null;
			dkcobj.offCasIncDec();
	...
	//
	it('k2hdkc - k2hdkc::CasIncDec(onetime) - inline Callback', function(done){
		const	dkcobj = k2hdkc();
		...
		// increment cas value
		expect(dkcobj.casIncDec(slvconf, slvport, true, false, 'casincdec-onetime-key-924', true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;
			...
			// decrement cas value
			expect(dkcobj.casIncDec(slvconf, slvport, true, false, 'casincdec-onetime-key-924', false, 'mypass', 600, function(error: any)
			{
				expect(error).to.be.null;
	...
	//
	it('k2hdkc - k2hdkc::CasIncDec(permanent) - No Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::CasIncDec(permanent) - on Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::CasIncDec(permanent) - onXXX Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
	...
	//
	it('k2hdkc - k2hdkc::CasIncDec(permanent) - inline Callback', function(done){
		const	dkcobj = k2hdkc(slvconf, slvport, true, false);
		...
		// increment cas value
		expect(dkcobj.casIncDec('casincdec-permanent-key-928', true, 'mypass', 600, function(error: any)
		{
			expect(error).to.be.null;
			...
			// decrement cas value
			expect(dkcobj.casIncDec('casincdec-permanent-key-928', false, 'mypass', 600, function(error: any)
			{
				expect(error).to.be.null;
	...
	// k2hdkc::PrintVersion
	//
	it('k2hdkc - k2hdkc::PrintVersion', function(done){
		const	dkcobj	= k2hdkc();
		...
		// fd
		const	fd = fs.openSync('/tmp/k2hdkc_test_printversion.log', 'w');
